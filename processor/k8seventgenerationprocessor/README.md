...
The `k8seventgenerationprocessor` parses Kubernetes events and manifests to generate OTel entities and relationships.

Supported inputs:
- `Pod` manifests: Generates `Container` and `ContainerImage` entities, and `Container` -> `ContainerImage` relationships.
- `Endpoints` and `EndpointSlice` manifests: Generates Service Mappings.
- `VulnerabilityReport` manifests (aquasecurity.github.io/v1alpha1): Generates `VulnerabilityDetail` entities and `VulnerabilityFinding` relationships.
