...
func (cp *k8seventgenerationprocessor) processLogs(_ context.Context, ld plog.Logs) (plog.Logs, error) {
	...
	if containersLogSlice.Len() > 0 {
		rl := addContainerResourceLog(ld)
		lrs := rl.ScopeLogs().At(0).LogRecords()
		containersLogSlice.CopyTo(lrs)
	}

	if serviceMappingsLogSlice.Len() > 0 {
		rl := addServiceMappingsResourceLog(ld)
		lrs := rl.ScopeLogs().At(0).LogRecords()
	...
	for res := range resCh {
		...
		case *manifests.PodManifest:
			...
			for i := range containers.Len() {
				lr := containers.At(i)
				lr.CopyTo(lrsContainers.AppendEmpty())
			}
		case *manifests.EndpointManifest, *manifests.EndpointSliceManifest:
			mappings := transformManifestToServiceMappingLogs(m.(manifests.ServiceMapping), res.Timestamp)
			for i := range mappings.Len() {
				lr := mappings.At(i)
				lr.CopyTo(lrsServiceMappings.AppendEmpty())
			}
		}
	}
}
