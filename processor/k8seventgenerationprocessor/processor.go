...
func (cp *k8seventgenerationprocessor) generateLogRecords(resCh <-chan result, entityStateEvents plog.LogRecordSlice, lrsServiceMappings plog.LogRecordSlice) {
	for res := range resCh {
		...
		case *manifests.PodManifest:
			...
			cicdContainerImages := transformContainersToCICDContainerImageLogs(manifestContainers, res.Timestamp, cp.logger)
			cicdContainerImages.MoveAndAppendTo(entityStateEvents)
			cicdRelatesTo := transformContainersToCICDContainerImageRelatesToLogs(manifestContainers, res.Timestamp, cp.logger)
			cicdRelatesTo.MoveAndAppendTo(entityStateEvents)
