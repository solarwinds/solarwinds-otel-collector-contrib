		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "swo.hostinfo.firewall":
					...
					assert.True(t, ok)
					assert.Equal(t, "firewall.profile.name-val", attrVal.Str())
				case "swo.hostinfo.uptime":
					...
					assert.True(t, ok)
					assert.Equal(t, "hostdetails.domain-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("hostdetails.domain.fqdn")
					assert.True(t, ok)
					assert.Equal(t, "hostdetails.domain.fqdn-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.Equal(t, "hostdetails.model.serialnumber-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("hostdetails.model.manufacturer")
					assert.True(t, ok)
					assert.Equal(t, "hostdetails.model.manufacturer-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("hostdetails.model.name")
					assert.True(t, ok)
					assert.Equal(t, "hostdetails.model.name-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.Equal(t, "hostdetails.timezone.caption-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("hostdetails.timezone.standardname")
					assert.True(t, ok)
					assert.Equal(t, "hostdetails.timezone.standardname-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.hostname")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.hostname-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.Equal(t, "osdetails.os-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.platform")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.platform-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.platform.family")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.platform.family-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.platform.version")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.platform.version-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.kernel.version")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.kernel.version-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.kernel.architecture")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.kernel.architecture-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.virtualization.system")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.virtualization.system-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.virtualization.role")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.virtualization.role-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.host.id")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.host.id-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.language.lcid")
					assert.True(t, ok)
					assert.EqualValues(t, 23, attrVal.Int())
					attrVal, ok = dp.Attributes().Get("osdetails.language.name")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.language.name-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("osdetails.language.displayname")
					assert.True(t, ok)
					assert.Equal(t, "osdetails.language.displayname-val", attrVal.Str())
				case "swo.hostinfo.user.lastLogged":
					...
					assert.True(t, ok)
					assert.Equal(t, "user.displayname-val", attrVal.Str())
				}
			}
		})
	}
